...
\begin{proof}
    We begin by estimating the error for $\xi_{0,h}$ in $H^1(\Omega)'$ using a duality argument. To this end, consider the following dual problem. Given $\chi \in H^1(\Omega)$, let $(\lambda, \mu) \in H^1(\Omega) \times H_0^1(\Omega)$ solve
    ...
    Thus combining \eqref{eq:err.coupled.xi0.1.a} and \eqref{eq:err.coupled.xi0.1.b.final} in \eqref{eq:err.coupled.xi0.1} leads to the following estimate,
    \begin{align}\label{eq:err.coupled.xi0.final}
        |(\xi_0 - \xi_{0,h}, \chi)| \lesssim h^{\min{((\pi / \omega)-\epsilon,k)}} \norm{L^2(\Omega)}{\bs{f}} \norm{H^1(\Omega)}{\chi} \quad \forall \chi \in H^1(\Omega).
    \end{align}
