...
\begin{lemma}[Interpolation operator stability and approximation]\label{lem:ID.properties}
    ...
    \begin{align}
        ...
        \norm{L^2(D)}{v - \ID v} + \norm{L^2(D)}{\ID v - \Pi^1_{k,D} \ID v} &\lesssim h_D^{{\min(\delta,k)}+1} \seminorm{H^{1+\delta}(D)}{v}, \quad \forall v \in H^{1+\delta}(D)\label{eq:ID.approximation.L2}
    \end{align}   
\end{lemma}
...
\begin{remark}\label{coercivity.assum}
    \corr{}{}{[JT: To do - Making concrete Assumption 5.7]}
