...
\begin{assumption}\label{assum:coercivity}
    ...
    \begin{align*}
        \seminorm{H^1(\Omega)}{v_h} &\lesssim \norm{h_\beta}{v_h} \quad \forall v_h \in V_h^0,  \\
        \seminorm{H^1(\Omega)}{v_h} &\lesssim \norm{h_\beta}{v_h} \quad \forall v_h \in V_h \quad \mbox{with} \quad (v_h,1) = 0. 
    \end{align*}
\end{assumption}
...
\begin{remark}\label{coercivity.assum}
    The coercivity Assumption for $\norm{h_0}{\cdot}$ for different stabilization forms has been studied in detail in \cite[(4.4),(4.5),(4.16)]{brenner.sung:2018:virtual} for meshes with small edges and in \cite{brenner.guan.ea:2017:some} for meshes satisfying Assumption \ref{meshreg_assum}. The coercivity Assumption for $\norm{h_\beta}{\cdot}$ is a trivial consequence.
\end{remark}
