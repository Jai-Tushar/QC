...
\end{lemma}
\begin{proof}
    Given $\xi_h \in V_h^0$, let $\tilde{\phi} \in H^1(\Omega)$ solve the following auxiliary problem,
...
\begin{theorem}\label{thm:simplyconnected}
    The approximations $\xi_h$ and $\bs{u}_h$ defined in \eqref{uh.simplyConnected} obtained by $\Poly_k$-virtual finite element method satisfy 
...
% The numerator in the last term of \eqref{err.abs.xi0h} is estimated for $1 \leq l \leq k$ using the definition of $\Pi^0_{k,h}$ and using the fact that it minimizes it's respective norm as follows:
% \begin{align}
%     (\rho_h, w_h - \Pi^0_{k,h} w_h) &= \sum_{D \in \Th} (\rho_h - \Pi^0_{l-2,D} \rho_h, w_h - \Pi^0_{k,D} w_h) \nonumber\\
%     &\leq \norm{L^2(\Omega)}{\rho_h - \Pi^0_{l-2,h} \rho_h} \norm{L^2(\Omega)}{w_h - \Pi^0_{0,h} w_h} \nonumber\\
%     &\lesssim h \norm{L^2(\Omega)}{\rho_h - \Pi^0_{l-2,h} \rho_h} \norm{H^1(\Omega)}{w_h}.
% \end{align}


% The term $a_h(\tilde \xi_0,v_h)$ in \eqref{err.abs.xi0h} can be rewritten as
% \begin{align}\label{eq:err.xi0h}
%     a_h(\tilde \xi_0, v_h) &= \sum_{D \in \Th} (\curl{\Pi^{1}_{k,D} \tilde \xi_0}, \curl{\Pi^1_{k,D} \eta_h}) + S^D((I - \Pi^1_{k,D}) \tilde \xi_0, (I - \Pi^1_{k,D}) \eta_h) \nonumber\\
%     &= \sum_{D \in \Th} (\curl{\Pi^{1}_{k,D} \tilde \xi_0}, \curl{\eta_h}) + S^D((I - \Pi^1_{k,D}) \tilde \xi_0, (I - \Pi^1_{k,D}) \eta_h) \nonumber\\
%     &= \sum_{D \in \Th} (\curl{(\Pi^{1}_{k,D} \tilde \xi_0 - \tilde \xi_0)}, \curl{(\eta_h - \Pi^1_{k,D} \eta_h})) + S^D((I - \Pi^1_{k,D}) \tilde \xi_0, (I - \Pi^1_{k,D}) \eta_h) \nonumber\\
%     &\qquad\qquad + (\rho_h, \eta_h) - \beta(\tilde \xi_0, \eta_h).
% \end{align}
% To get the last equality, we used the definition of $\Pi^1_{k,D}$ and \eqref{eq:P.aux}. The first two terms in \eqref{eq:err.xi0h} can be estimated using Cauchy-Schwarz inequality and definition of $\norm{h_\beta}{\cdot}$-norm, to get
% \begin{align}
%     \sum_{D \in \Th} (\curl{(\Pi^{1}_{k,D} \tilde \xi_0 - \tilde \xi_0)}, \curl{(\eta_h - \Pi^1_{k,D} \eta_h)}) \lesssim \seminorm{1,h}{\Pi^1_{k,h} \tilde{\xi}_0 - \tilde \xi_0} \norm{h_\beta}{\eta_h} \label{eq:err.xi0h.a}\\
%     \sum_{D \in \Th}  S^D((I - \Pi^1_{k,D}) \tilde \xi_0, (I - \Pi^1_{k,D}) \eta_h) \lesssim \norm{h_\beta}{\tilde \xi_0 - \Pi^1_{k,D} \tilde \xi_0} \norm{h_\beta}{\eta_h} \label{eq:err.xi0h.s}
% \end{align}
% On substituting \eqref{eq:err.xi0h} in \eqref{err.abs.xi0h}, we can estimate the rest of the terms using $\Pi^0_{k,h}$,
% \begin{align}
%     \beta(\tilde \xi_0, \Pi^0_{k,h} \eta_h) - \beta(\tilde \xi_0, \eta_h)
% \end{align} 
