...
\begin{lemma}\label{lem:est.phi.mc}
...
\begin{proof}
    ...
    \end{align}
    where $\lambda_{\min}(\bs{A})$ denotes the minimum eigenvalue of $\bs{A}$. In order to control the $\norm{\infty}{\bs{A}_h}$, we write $\bs{A}^{-1}_h = (\bs{I} - \bs{A}^{-1} (\bs{A}_h - \bs{A}))^{-1} \bs{A}^{-1}$, and proceed using the Von Neumann lemma and \eqref{eq:err.est.A.b} as follows,
    ...
    \end{align}
    The estimate \eqref{eq:err.est.cj.Ahinv} holds provided $\norm{\infty}{\bs{A}^{-1}(\bs{A}_h - \bs{A})} < 1$. On simplification this leads to the following condition on $h$:
    \begin{align*}
