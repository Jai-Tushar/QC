...
\begin{lemma}\label{lem:est.mesh.dependent}
    ...
    \begin{align}
        ...
        \norm{h_\beta}{v- \Ih{v}} + \norm{h_\beta}{v- \Pi^1_{k,h} v} &\lesssim \quad \forall v \in H^{1+\delta}(\Omega). \label{eq:est.hbeta.norm}
    \end{align}
\end{lemma}
...
\begin{proof}
    ...
    \end{align}
    It follows now from \eqref{eq:P.aux}, \eqref{eq:P.xi0}, and the duality estimate \eqref{eq:err.est.rho} that
    \begin{align}\label{eq:xi0.a}
        \seminorm{H^1(\Omega)}{\xi_0 - \tilde{\xi}_0} &\lesssim h^{\min{((\pi/\omega) - \epsilon, k)}} \norm{L^2(\Omega)}{\bs{f}}.
    \end{align}

It follows from \eqref{eq:P.aux} and \eqref{eq:Ph.xi0h} that $\xi_{0,h}$ is also a virtual element approximation of $\tilde \xi_0$ and hence we have the following analogus abstract error estimate following \cite[Section 4.3]{Brenner-Sung-2018-VEM-Small-Edges-Faces},
