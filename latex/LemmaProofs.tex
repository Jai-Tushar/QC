...
We have the following two properties associated with this choice of stabilization (see Remark 4.3 \cite{brenner.sung:2018:virtual}),
\begin{align}
...
\begin{align*}
    ...
    &\overset{\eqref{eq:stab.p2}}{\lesssim} \sum_{D \in \Th} \norm{L^\infty(D)}{(I - \Pi^1_{k,h}) \Ih v}^2 \\
...
Using the linearity and idempotency of $\Pi^1_{k,h}$, we can get the following
\begin{align*}
    \norm{h_\beta}{v- \Pi^1_{k,h}{v}}^2 &= \sum_{D \in \Th} \norm{L^\infty(\partial D)}{v - \Pi^1_{k,D} v}^2 + \beta \norm{L^2(D)}{v - \Pi^1_{k,D} v}^2 \\
...
\begin{align*}
    ...
    &\lesssim \sum_{D \in \Th} \seminorm{H^1(D)}{v - \ID v}^2 + \norm{L^\infty(D)}{(I-\ID{})v}^2 +  \norm{L^\infty(D)}{\Pi^1_{k,D}(I-\Pi^1_{k,D})v}^2 
    + \beta \norm{L^2(D)}{v - \ID v}^2,
\end{align*}    
where we used the stability of projectors \eqref{eq:stability.H1p}, \eqref{eq:stability.L2p} and the property \eqref{eq:stab.p2}. The first and last terms are bounded using the approximation properties of $\ID$ from Lemma \ref{lem:ID.properties}. The second term is bounded using the Sobolev inequality \eqref{eq:Sobolev.inequality} followed by the approximation properties of $\ID$ from Lemma \ref{lem:ID.properties}. The third term can be estimated as in \cite[(4.24)]{brenner.sung:2018:virtual}. Combining all the resulting bounds leads to the desired estimate on the second term in \eqref{eq:est.hbeta.norm}.
