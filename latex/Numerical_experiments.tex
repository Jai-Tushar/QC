...
We choose the classical \texttt{Dofi-Dofi} definition of the stabilization term (see \cite{beirao-da-veiga.brezzi.ea:2013:basic}),
\begin{align}
    S^D(w_h, v_h) = \sum_{i=1}^{N_D^{\texttt{dof}}} \chi_i(w_h) \; \chi_i(v_h) \quad \forall w_h, v_h \in V_h(D),
\end{align}
where operator $\chi_i(\cdot)$ associates the function with its $i$-th degree of freedom, and $N_D^{\texttt{dof}}$ is the number of degrees of freedom associated with an element in $D \in \Th$.
...
\subsection{Smooth solution on a convex simply connected domain.}\label{exp:smooth_sol}
    ...
    \begin{align*}
        ...
        e_{\xi_h} := \seminorm{1,h}{\curl{\bs{u}} - \Pi^{1}_{k,h} \xi_h} &= \sqrt{\sum_{D \in \Th} \seminorm{H^1(D)}{\curl{\bs{u}} - \Pi^{1}_{k,D} \xi_h}^2},
    \end{align*}
